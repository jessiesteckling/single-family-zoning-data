...
| A3 | Socrata GeoJSON is WGS84 when no `crs` member is present | Holds, and is explicit | RFC 7946 mandates it, and Socrata sends no `crs` member, so the CRS is passed to `GeoDataFrame.from_features` rather than inferred |
